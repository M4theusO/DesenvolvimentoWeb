...
$(document).ready(function(){
		...
		$.ajax({
			...
			success: function(marcas){
				...
				if(marcas!="" && valida == true){
					...
					}else if(marcas!="" && valida == false){
						
						$(select).html("");
						
						var option = document.createElement("option");
						option.setAttribute("value", "");
						option.innerHTML = ("Nenhuma marca ativa!");
						$(select).append(option);
		...
		if((produto.categoria=="")||(produto.marcaId=="")||(produto.modelo=="")||(produto.capacidade=="")||(produto.valor=="")){
			...
			$.ajax({
				...
				success: function(msg){
						...
						COLDIGO.produto.buscar();//recarrega a tabela
	...
	COLDIGO.produto.editar = function(){
		...
		$.ajax({
			...
			data: JSON.stringify(produto), //dados a serem enviados
			success: function(msg){
				COLDIGO.exibirAviso(msg);
				COLDIGO.produto.buscar();
	...
	COLDIGO.produto.confirmaExclusao = function(id){
		...
		$("#modalAviso").html("Você realmente deseja excluir esse produto?");
